...
rm(list = ls())
hpc <- "/data/xwang/adsp3"
github <- "~/Dropbox/GitHub/wgs2"
...
# load(paste("./QTLRel/", chr, ".rdt", sep = ""))
  load(paste("./QTLRel2/", chr, ".rdt", sep = ""))
  fit.p <- lapply(fit, function (x) x$p) %>% unlist
  fit.v <- lapply(fit, function (x) x$v) %>% unlist
  fit.e <- lapply(fit, function (x) do.call(rbind, x$parameters))
  fit.e <- do.call(rbind, fit.e)
  mutate(as.data.frame(fit.e), LOD = fit.p, V = fit.v)
}); names(lmm.fit) <- chr

save(lmm.fit, file = "QTLRel.rdt")
save(lmm.fit, file = "QTLRel2.rdt")
